...
describe('frontier strategy balance', () => {
  ...
  it('makes the frontier controller spend its finite pool on training', () => {
    ...
    expect(frontier.training + frontier.inference + frontier.research).toBeCloseTo(1, 12)
  ...
  it('applies integrated research to rival training on the player formula', () => {
    ...
    expect(integrated.trainEfficiency).toBeGreaterThan(baseline.trainEfficiency)
  ...
  it('finalizes rival runs with their manifest recipe and a domain vector', () => {
    ...
    for (const rival of released) {
      ...
      expect(manifest?.domainWeights).toMatchObject(
        rivalTrainingWeights(rival.archetype),
      )
    ...
    () => {
      ...
      const rows = collectQuarterlyLabSnapshots(report.final).map((row) => {
        ...
        return {
          ...
          research: rival?.researchUnlocked.length ?? report.final.player.researchUnlocked.length,
          ...
          latestAudit: audits.toSorted((a, b) => b.publishDay - a.publishDay)[0]?.scores,
        }
      })
      console.log(
        JSON.stringify({
