 ...
 */
import type { MapCity, MapRegion, MapTile, Model, ModelFinanceRow, RackDesign, SimState } from './types'
...
function restoreState(stateRaw: unknown, snapshot: DynamicWorldSnapshotV2 | null): SimState {
  ...
  if (restored.lastMarket) {
    ...
    restored.lastMarket = {
      ...
      modelFinance: (ensureArray(restored.lastMarket.modelFinance) as ModelFinanceRow[]).map((row) => {
