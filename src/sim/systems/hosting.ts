...
export function modelHostNeed(m: Model): ModelHostNeed {
  ...
  const activeB = Math.max(0.01, isMoe ? (m.activeParamsB ?? m.paramsB * 0.1) : m.paramsB)
  ...
  if (isMoe && totalB > activeB * 4) {
    ...
    ramBias = Math.max(ramBias, Math.min(0.88, 0.45 + Math.log10(totalB / activeB) * 0.22))
  }
...
function publicModels(state: SimState): Model[] {
  return state.player.models.filter((m) => m.release === 'released' || m.shipped)
}
...
export function fleetHostSnapshot(state: SimState): FleetHostSnapshot {
  ...
  if (models.length === 1) {
    ...
    pfNeed = sorted[0]!.hostPf + sorted.slice(1).reduce((s, m) => s + m.hostPf * 0.35, 0)
  }
  ...
  const trafficPf =
    ...
    Math.min(state.lastMarket?.demandPf ?? 0, state.lastMarket?.capacityPf ?? 0)
  ...
  const shortCompute = effectivePfNeed > 0.01 && pfServe < effectivePfNeed * 0.85
  ...
  if (shortOn === 'vram' || (models[0] && models[0].ramBias > 0.55)) {
    ...
    recommendedSkuReason = 'High-VRAM H2-Node — better for large weight residency.'
  } else if (shortOn === 'compute' || (models[0] && models[0].computeBias > 0.6)) {
    recommendedSkuId = 'rack_infer'
    recommendedSkuReason = 'Serve Sled — more tokens/sec per bay for smaller models.'
  } else if (models[0] && (models[0].ramBias > 0.6 || models[0].paramsB >= 400)) {
    recommendedSkuId = 'rack_h200'
...
export function autoBalanceHosting(state: SimState): SimState {
  ...
  const targetServePf = Math.min(pfHave * 0.85, Math.max(trafficNeed * 1.15, pfHave * 0.25))
  ...
  if (sku && halls.length > 0 && (host.shortOn !== 'ok' || host.computeUtil < 0.65)) {
    ...
    if (host.vramNeed > host.vramHave) {
      ...
      needBays = Math.max(needBays, Math.ceil(vramGap / Math.max(1, sku.vramGb)))
    }
    if (host.pfNeed > 0 && host.computeUtil < 0.8) {
      const pfGap = host.pfNeed * 0.85 - host.pfServe
      if (pfGap > 0) {
        needBays = Math.max(needBays, Math.ceil(pfGap / Math.max(0.05, sku.flopsPf)))
      }
    }
