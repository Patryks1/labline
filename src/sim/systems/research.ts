...
export function nodePrereqDepth(nodeId: string, seen = new Set<string>()): number {
...
export function researchPfTargetForNode(
  ...
  const mult = (ECONOMY.researchPfCostMult ?? 2.6) * Math.max(0.4, researchCostMult)
...
export function researchPfTarget(state: SimState, node: ResearchNodeDef): number {
...
export function canLabResearchNode(
  ...
  if (unlocked.includes(nodeId)) return { ok: false, reason: 'Already unlocked' }
  if (researchers < 1) {
  ...
  if (node.exclusiveWith) {
    for (const e of node.exclusiveWith) {
      if (unlocked.includes(e)) {
        return { ok: false, reason: `Conflicts with ${getResearchNode(e).name}` }
      }
    }
  }
...
export function researchCashEstimate(state: SimState, node: ResearchNodeDef): number {
...
export function nodeVisualStatus(state: SimState, nodeId: string): NodeVisualStatus {
...
export function canEnqueue(state: SimState, nodeId: string): { ok: boolean; reason?: string } {
...
export function canStartNow(state: SimState, nodeId: string): { ok: boolean; reason?: string } {
  if (state.player.researchUnlocked.includes(nodeId)) {
    return { ok: false, reason: 'Already unlocked' }
  }
...
export function enqueueResearch(state: SimState, nodeId: string): SimState {
  ...
  const plan = planResearchPath(state.player.researchUnlocked, scheduled, nodeId)
  if (plan.reason) {
    return {
      ...state,
      alerts: [
...
export function moveQueue(state: SimState, nodeId: string, dir: -1 | 1): SimState {
...
>(lab: T, effects: ResearchEffects): T {
  ...
  if (effects.servingEfficiency)
    p.servingEfficiency = Math.min(1.45, p.servingEfficiency + effects.servingEfficiency)
  if (effects.trainEfficiency != null && p.trainEfficiency != null) {
    p.trainEfficiency = Math.min(1.5, p.trainEfficiency + effects.trainEfficiency)
  }
  ...
  if (p.dataQuality != null) {
    ...
    if (effects.dataFlywheel) {
      p.dataQuality = Math.min(2.8, p.dataQuality + effects.dataFlywheel * 0.15)
    }
  }
  return p
}

function applyEffects(state: SimState, effects: ResearchEffects): SimState {
...
export function aggregateEffects(unlocked: string[]): ResearchEffects {
  ...
  for (const id of unlocked) {
    ...
    if (e.trainEfficiency) acc.trainEfficiency = (acc.trainEfficiency ?? 0) + e.trainEfficiency
    if (e.energyPue) acc.energyPue = (acc.energyPue ?? 0) + e.energyPue
    if (e.capabilityBonus) acc.capabilityBonus = (acc.capabilityBonus ?? 0) + e.capabilityBonus
    if (e.moeInferMult) acc.moeInferMult = (acc.moeInferMult ?? 1) * e.moeInferMult
    if (e.denseInferMult) acc.denseInferMult = (acc.denseInferMult ?? 1) * e.denseInferMult
    ...
    if (e.talentAttract) acc.talentAttract = (acc.talentAttract ?? 0) + e.talentAttract
    ...
    if (e.trainingStumbleRisk)
      acc.trainingStumbleRisk = (acc.trainingStumbleRisk ?? 0) + e.trainingStumbleRisk
...
export function tickResearch(state: SimState): SimState {
  ...
  let labMult = 1
  for (const t of facilityAnchorTiles(s, { ownerId: 'player' })) {
    if (
      t.kind === 'lab' &&
      t.buildingProgress >= t.buildingTarget
    ) {
  ...
  if (progress <= 0 && (researchers < 1 || researchers < needR) && s.day % 4 === 0) {
    s = {
      ...s,
      alerts: [
        {
  ...
  s = {
    ...
    player: {
      ...s.player,
      cash: Math.max(0, s.player.cash - cashBurn),
      activeResearch: next,
      /** Today’s research $ burn (market does not re-add; cash already reduced) */
