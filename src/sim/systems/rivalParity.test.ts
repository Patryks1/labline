...
describe("rival compute parity", () => {
  ...
  it("caps each job step at both available work and remaining work", () => {
    ...
    expect(
      progressRivalTrainingJob(job, Number.POSITIVE_INFINITY).workAppliedPfDays,
    ).toBe(0);
  });

  it("applies the same trillion-scale pacing ceiling to rival jobs", () => {
    const job = {
      ...activeJob("rival_trillion"),
      paramsB: 1_000,
      targetPfDays: 12_000,
      progressPfDays: 0,
      minCalendarDays: 120,
    };
    expect(progressRivalTrainingJob(job, 1_000)).toMatchObject({
      workAppliedPfDays: 100,
      job: { progressPfDays: 100, daysElapsed: 1 },
    });
  });
  ...
  it("freezes researched and hardware-compatible numerics onto new rival jobs", () => {
    ...
    const efficiency = next.rivals.find(
      (rival) => rival.archetype === "efficiency",
    );
