...
export type ModelFamily = 'dense' | 'moe' | 'diffusion' | 'video' | 'omni' | 'embedding'
...
export interface SiteProject {
  ...
  completedDay?: number
  cancelledDay?: number
  status: 'planning' | 'grid_queue' | 'construction' | 'commissioning' | 'complete' | 'cancelled'
}
...
export type EvaluationKind = 'internal' | 'public' | 'blind_audit' | 'real_world'
...
export type LabAction =
  ...
  | { kind: 'set_api_precision'; modelId: string; precision: PlanServePrecision }
  | { kind: 'set_training_priority'; jobId: string; priority: number; reservedPf?: number }
...
export type LabIntent =
  | { id: string; labId: LabId; kind: 'allocation'; allocation: Allocation; submittedDay: number }
  | { id: string; labId: LabId; kind: 'resource_order'; order: ResourceOrder; submittedDay: number }
  | { id: string; labId: LabId; kind: 'loan_application'; application: LoanApplication; submittedDay: number }
  | { id: string; labId: LabId; kind: 'api_price'; modelId: string; priceIn: number; priceOut: number; submittedDay: number }
