...
export function KpiHistoryPopover({
  ...
  const areaPath = points.length > 0
    ...
    : ''
  const selectedIndex = hoverIndex == null
  ...
  return (
    <section
      className="pointer-events-auto fixed left-1/2 top-[3.75rem] z-[90] w-[min(44rem,calc(100vw-1rem))] -translate-x-1/2 rounded-2xl border border-line bg-panel/95 p-3 shadow-2xl backdrop-blur-xl"
      aria-label={`${definition.label} history`}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 items-center gap-2">
          <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-mint/10 text-mint">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate font-mono text-[0.6875rem] text-muted">
              D{points[0]?.day ?? current.day}–D{points.at(-1)?.day ?? current.day} · {points.length} daily closes
            </p>
          </div>
        </div>
        <button type="button" aria-label="Close metric history" onClick={onClose} className="rounded-lg p-1.5 text-muted hover:bg-void hover:text-bone">
          <X size="1rem" />
        </button>
      </div>
      ...
      <div className="mt-3 grid grid-cols-[1fr_auto] items-end gap-3">
        <div>
          ...
          <div className={`mt-0.5 font-mono text-[0.6875rem] ${delta < 0 ? 'text-danger' : 'text-mint'}`}>
            {delta >= 0 ? '+' : ''}{definition.format(delta)} over selected range
          </div>
        </div>
        <div className="flex rounded-lg border border-line bg-void/70 p-0.5" role="group" aria-label="History range">
          {RANGE_OPTIONS.map((days) => (
            <button
              ...
              className={`rounded-md px-2 py-1 font-mono text-[0.6875rem] ${range === days ? 'bg-mint/15 text-mint' : 'text-muted hover:text-bone'}`}
            >
              {days}D
            </button>
          ))}
      ...
      <div className="relative mt-2.5 h-44 overflow-hidden rounded-xl border border-line/70 bg-void/55 px-2 py-2">
        {points.length > 1 ? (
          <>
            ...
            >
              ...
              <path d={linePath} fill="none" stroke={definition.color} strokeWidth="2.5" vectorEffect="non-scaling-stroke" />
              {selected ? (
                <>
                  <line x1={xFor(selectedIndex)} x2={xFor(selectedIndex)} y1="0" y2={height} stroke={definition.color} strokeOpacity="0.35" strokeDasharray="4 4" vectorEffect="non-scaling-stroke" />
                  <circle cx={xFor(selectedIndex)} cy={yFor(selected.value)} r="4" fill={definition.color} stroke="#071217" strokeWidth="2" vectorEffect="non-scaling-stroke" />
                </>
              ) : null}
            </svg>
            {selected ? (
              <div
                ...
                style={{ left: `${points.length <= 1 ? 50 : (selectedIndex / (points.length - 1)) * 100}%` }}
              >
          ...
          <div className="flex h-full items-center justify-center text-center text-[0.75rem] text-muted">
            {metric === 'brand' ? 'Brand history starts recording now.' : 'Advance the simulation to build a trend.'}
          </div>
        )}
      </div>

      <div className="mt-2 flex items-center justify-between gap-3">
        <p className="text-[0.6875rem] text-muted">Hover the line for an exact daily close.</p>
        <button type="button" onClick={onOpenDetails} className="text-[0.75rem] font-medium text-mint hover:text-bone">
          Open detailed view →
        </button>
      </div>
    </section>
