...
export function ResearchUnlockLink({ nodeId, label, compact = false, className = '' }: Props) {
  ...
  return (
    <button
      ...
      className={`${
        compact
          ? 'inline-flex items-center gap-1 text-[0.6875rem] font-medium text-research hover:text-bone'
          : 'flex w-full items-center justify-between gap-2 rounded-lg border border-research/35 bg-research/10 px-2.5 py-2 text-left text-[0.75rem] text-research transition hover:border-research/65 hover:bg-research/15 active:translate-y-px'
