...
export function SliderField({
  ...
  return (
    <label className="group/slider relative block">
      {hoverContent && (
        <div
      ...
      <div className="mb-0.5 flex items-baseline justify-between gap-2 text-[0.75rem]">
        ...
        <span className={`font-mono ${accentClass}`}>{format(value)}</span>
      </div>
      {sublabel && (
        <div className="mb-0.5 font-mono text-[0.6875rem] leading-tight text-muted/90">{sublabel}</div>
      )}
